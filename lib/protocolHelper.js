...
function getOSData() {
	...
	var osVersion, osPlatformHuman, osName;
	if (osPlatform === 'win32' || osPlatform === 'darwin') {
		osVersion = os.release();
		var osVersionShort = (osPlatform === 'win32') ? osVersion.split('.')[0] : osVersion.split('.')[1];

		osPlatformHuman = osMap.platform[osPlatform] || 'Unknown';
		osName = (osMap.name[osPlatform] && osMap.name[osPlatform][osVersionShort]) ? osMap.name[osPlatform][osVersionShort] : 'Unknown';
	} else {
    getos(function (err, info) {
	    if(err) console.log(err);
	    osPlatformHuman = info.os;
	    osName = info.dist;
...
protocolHelper.createSpeechConfigPacket = function(connectionId) {
  ...
  //populate the os details in speech config before sending
  speechConfig.context.os = getOSData();
