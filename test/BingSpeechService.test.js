...
test('[ms-bing-speech-service] method _start', function (t) {
  ...
  recognizer.start().then((service) => {
    t.equals(typeof service.sendFile, 'function', 'service object has sendFile function');
  }).catch(console.error);
});
...
test('[ms-bing-speech-service] method onMessage', function (t) {
  ...
  recognizer.start().then((service) => {
    ...
    service.on('turn.start', () => t.pass('turn.start: event fired'));
    ...
    service.on('speech.startDetected', () => t.pass('speech.startDetected: event fired'));
    ...
    service.on('speech.endDetected', () => t.pass('speech.endDetected: event fired'));
    ...
    service.on('turn.end', () => t.pass('turn.end: event fired'))
    ...
    t.notOk(service.turn.active, 'turn.end service turn.active prop is false');
...
test('[ms-bing-speech-service] method stop', function (t) {
  ...
  recognizer2.start().then((service) => {
    service.close();
...
test('[ms-bing-speech-service] service event handlers', function (t) {
  ...
  const recognizer = new speechService({accessToken: 'abc'});
  recognizer.start().then((service) => {
    t.ok(service.onmessage, 'service has onmessage handler');
    t.ok(service.onopen, 'service has onopen handler');
    t.ok(service.onclose, 'service has onclose handler');
    t.ok(service.onerror, 'service has onerror handler');
  });
});
